...
const mutationHandler: MutationHandler<CreateImageUploadInput, ImageUpload> = async ({ input: { filename }, identity, trx }) => {
...
async function createImagePresignedPostParams(trx: Transaction<Database>, imageId: string, filename: string): Promise<aws.S3.PresignedPost.Params> {
  const mimeType = mime.lookup(filename);
  if (mimeType && mimeTypes.includes(mimeType)) {

    const key = `raw/${imageId}.${mime.extension(mimeType)}`
    ...
    await trx.insertInto('image')
      .values(imageValues)
      .executeTakeFirstOrThrow();
  
    return {
      Expires: uploadExpiration,
      Bucket: assertDefinedReturn(process.env.BUCKET_NAME),
